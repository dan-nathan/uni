...
public abstract class StatementNode {
    ...
    public static class ForNode extends StatementNode {
        // The identifier for the loop variable variable
        private final String ctrlId;

        // The variable node entry for the loop control variable
        private ExpNode.VariableNode ctrlVar;

        // The upper and lower bounds of the loop
        ...
        public ForNode(Location loc, String ctrlId, ExpNode lower, ExpNode upper,
                   List<StatementNode> loopStmt, Location loopStmtLoc) {
            super(loc);
            this.ctrlId = ctrlId;
            this.lower = lower;
            this.upper = upper;
            this.loopStmt = new ListNode(loopStmtLoc, loopStmt);
        }
        ...
        public ExpNode.VariableNode getCtrlVar() {
            return ctrlVar;
        }

        public void setCtrlVar(ExpNode.VariableNode var) {
            this.ctrlVar = var;
        }

        @Override
        public String toString(int level) {
            return "FOR " + ctrlId + " : " + lower.toString() + ".." + upper.toString() + " DO" +
                    newLine(level + 1) + loopStmt.toString(level + 1);
        }
    }
}
