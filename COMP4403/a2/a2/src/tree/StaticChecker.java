...
public class StaticChecker implements DeclVisitor, StatementVisitor,
    ...
    public void visitAssignmentNode(StatementNode.AssignmentNode node) {
        ...
        if (left.getType() instanceof Type.ReferenceType) {
            // Check if the variable being assigned to is read only
    ...
    public void visitForNode(StatementNode.ForNode node) {
        beginCheck("For");
        // Ensure types match
        ExpNode newLower = node.getLower().transform(this);
        ExpNode newUpper = node.getUpper().transform(this);
        /* Ensure the type is not a reference type (which aren't scalar types), and can work across
         * Different subranges
         */
        Type lowerType = newLower.getType().optDereferenceType().optWidenSubrange();
        newLower = lowerType.coerceExp(newLower);
        node.setLower(newLower);
        /* Ensure the bounds have compatible types. Use coerceToType instead of coerceExp so that
         * an error message can given for the for loop having incompatible types, and for the upper
         * type failing to coerce
         */
        try {
            node.setUpper(lowerType.coerceToType(newUpper));
        } catch (IncompatibleTypes e) {
            staticError("types of upper and lower bounds must match",
                    node.getLower().getLocation());
            errors.debugMessage("******" + e.getMessage());
            errors.error(e.getMessage(), node.getUpper().getLocation());
        }

        // Check that the lower bound (and hence also the upper bound) is a scalar type
        if (lowerType.optDereferenceType().getScalarType() == null) {
            staticError("For loop increment variable must be scalar type",
                    node.getLower().getLocation());
        }

        currentScope = currentScope.extendCurrentScope();
        // Add loop counter variable to current scope
        currentScope.addVariable(node.getCtrlId(), node.getLocation(),
                new Type.ReferenceType(node.getLocation(), lowerType));
        // Store loop counter variable in for statement
        node.setCtrlVar(new ExpNode.VariableNode(node.getLocation(),
                currentScope.lookupVariable(node.getCtrlId())));
        node.getCtrlVar().getVariable().setReadOnly(true);
        // Visit loop body
    ...
    /**
     * Handles a reference to an array index e.g. a[3]
     */
    public ExpNode visitArrayIndexNode(ExpNode.ArrayIndexNode node) {
        beginCheck("Array Index");
        ExpNode newIndex = node.getIndex().transform(this);
        SymEntry entry;
        // 1 dimensional array
        if (node.getId() instanceof ExpNode.IdentifierNode) {
            entry = currentScope.lookup(((ExpNode.IdentifierNode) node.getId()).getId());
            // If the entry doesn't exist this is already a syntax error for the identifier
            if (entry != null) {
                // If the entry is a reference to an array
                if (!(entry.getType() instanceof Type.ReferenceType)
                        || entry.getType().getArrayType() == null) {
                    staticError("Array type expected", node.getLocation());
                } else {
                    Type indexType = entry.getType().getArrayType().getArgType();
                    // Check index type
                    newIndex = indexType.coerceExp(newIndex);
                    // Set the type of the node (ensure it is a reference type that can be used by
                    // assignments)
                    node.setType(new Type.ReferenceType(node.getLocation(),
                            entry.getType().getArrayType().getResultType()));
                }
            }
            node.setId(node.getId().transform(this));
        } else if (node.getId() instanceof ExpNode.ArrayIndexNode) {
            // Visit and type check the next dimension down of the array
            ExpNode prevDimension = node.getId().transform(this);
            Type indexType = prevDimension.getType().getArrayType().getArgType();
            newIndex = indexType.coerceExp(newIndex);
            node.setType(new Type.ReferenceType(node.getLocation(),
                    prevDimension.getType().getArrayType().getResultType()));
        } else {
            staticError("Array identifier required", node.getLocation());
            endCheck("Identifier");
            return new ExpNode.ErrorNode(node.getLocation());
        }
        node.setIndex(newIndex);
        endCheck("Array Index");
        return node;
    }
