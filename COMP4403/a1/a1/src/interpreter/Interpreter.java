...
public class Interpreter implements StatementVisitor, ExpTransform<Value> {
    ...
    /**
     * Execute code for a branch of a do statement
     * Exit codes:
     *      0 - condition evaluated to false
     *      1 - condition evaluated to true with an exit statement
     *      2 - condition evaluated to true without an exit statment
     */
    public int visitDoBranchNode(StatementNode.DoBranchNode node) {
        beginExec("DoBranch");
        ExpNode condition = node.getCondition();
        if (condition.evaluate(this).getInteger() == Type.TRUE_VALUE) {
            // Execute statement if condition evaluates to true
        ...
        // If doesn't evaluate to true, exit with code 0
        endExec("DoBranch");
        return 0;
    }
    ...
    public void visitDoStatementNode(StatementNode.DoStatementNode node) {
        beginExec("DoStatement");
        boolean done = false;
        while (!done) {
            // Keeps track for if no branch runs
            boolean branchFound = false;
            for (StatementNode branch : node.getBranches()) {
                // Don't try to evaluate if it's an error node
                if (branch instanceof StatementNode.DoBranchNode) {
                    int branchExitStatus = ((StatementNode.DoBranchNode)branch).acceptInt(this);
                    switch (branchExitStatus) {
                        // Branch condition is true with exit statement - terminate while loop
                        case 1:
                            done = true;
                            // Branch condition is true without exit statement
                        case 2:
                            branchFound = true;
                    }
                    // Don't check other branches once one true one has been found
                    if (branchFound) {
                        break;
                    }
                }
