...
public class StaticChecker implements DeclVisitor, StatementVisitor,
    ...
    public void visitAssignmentNode(StatementNode.AssignmentNode node) {
        ...
        // Check for duplicate identifiers. Using HashSet for better time complexity
        Set<String> idSet = new HashSet<>();
        for (int i = 0; i < node.getLValues().size(); i++) {
            // These nodes are created as ExpNode.IdentifierNode in parseLValue, meaning they are
            // guaranteed to be able to be casted to ExpNode.IdentifierNode
            String id = ((ExpNode.IdentifierNode)node.getLValues().get(i)).getId();
            // Set.add() returns false if the element already exists in the set
            if (!idSet.add(id)) {
                staticError(id + " assigned more than once",
                        node.getLValues().get(i).getLocation());
            }
        }
        // Used to avoid IndexOutOfBoundsException when one list is bigger than the other
        int minListSize = Math.min(node.getLValues().size(), node.getConditions().size());
        for (int i = 0; i < minListSize; i++) {
            // Check the left side left value.
            ExpNode left = node.getLValues().get(i).transform(this);
            node.setLValue(i, left);
            ExpNode exp = node.getConditions().get(i).transform(this);
            node.setCondition(i, exp);
            if (left.getType() instanceof  Type.ReferenceType) {
                ...
                node.setCondition(i, baseType.coerceExp(exp));
    ...
    public int visitDoBranchNode(StatementNode.DoBranchNode node) {
        ...
        endCheck("DoBranch");
        // Return arbitrary value to be discarded
        return -1;
    }
