...
public class Parser {
    ...
    /**
     * Parse method for a list of expressions. Return an empty list if there's an error
     */
    private final ParseMethod<List<ExpNode>> expList = new ParseMethod<>(
            (Location loc) -> new LinkedList<>());
    ...
    private List<ExpNode> parseLValueList(TokenSet recoverSet) {
        return expList.parse("LValue List", LVALUE_START_SET, recoverSet,
                () -> {
                    Location loc = tokens.getLocation();
                    List<ExpNode> nodes = new LinkedList<>();
                    nodes.add(parseLValue(recoverSet.union(Token.COMMA)));
    ...
    private final ParseMethod<StatementNode.AssignmentNode> assign =
            new ParseMethod<>(
                    (Location loc) -> new StatementNode.AssignmentNode(loc,
                            new LinkedList<ExpNode>(), new LinkedList<ExpNode>()));
    ...
    private StatementNode.AssignmentNode parseAssignment(TokenSet recoverSet) {
        return assign.parse("MDAssignment", LVALUE_START_SET, recoverSet,
                () -> {
                     ...
                     */
                    Location loc = tokens.getLocation();
                    List<ExpNode> left = parseLValueList(recoverSet.union(
                            Token.ASSIGN, Token.EQUALS));
                    Location assignLoc = tokens.getLocation();
                    tokens.match(Token.ASSIGN, CONDITION_START_SET);
                    List<ExpNode> right = parseConditionList(recoverSet);
                    if (left.size() != right.size()) {
                        errors.error("number of variables does not match number of expressions in assignment", assignLoc);
                    }
    ...
    private StatementNode parseDoStatement(TokenSet recoverSet) {
        return stmt.parse("Do Statement", Token.KW_DO, recoverSet,
                () -> {
                    ...
                    List<StatementNode> branches = new LinkedList<>();
                    StatementNode branch = parseDoBranch(recoverSet
                            .union(Token.SEPARATOR, Token.KW_OD));
                    branches.add((StatementNode) branch);
                    while (tokens.isMatch(Token.SEPARATOR)) {
                        tokens.match(Token.SEPARATOR);
                        // Branch can be followed by '[]' or 'od'
                        branch = parseDoBranch(recoverSet.union(Token.SEPARATOR, Token.KW_OD));
                        branches.add((StatementNode) branch);
                    }
                    tokens.match(Token.KW_OD, recoverSet);
                    return new StatementNode.DoStatementNode(loc, branches);
                });
    }
    ...
    private StatementNode parseDoBranch(TokenSet recoverSet) {
        return stmt.parse("Do Branch", CONDITION_START_SET, recoverSet,
                () -> {
                    ...
                    return new StatementNode.DoBranchNode(loc, cond, stmtList, hasExit);
                });
    }
