...
public class StaticChecker implements DeclVisitor, StatementVisitor,
    ...
    public void visitProcedureNode(DeclNode.ProcedureNode node) {
        ...
        // Set the offsets of the parameters to the correct location
        int currentOffset = 0;
        for (SymEntry.ParamEntry param : procEntry.getType().getFormalParams()) {
            currentOffset -= param.getSpace();
            param.setOffset(currentOffset);
        }
    ...
    public void visitCallNode(StatementNode.CallNode node) {
        ...
        if (entry instanceof SymEntry.ProcedureEntry) {
            ...
            // Loop through the actual parameters and attempt to match with the formal parmaeters
            // of the procedure
            List<SymEntry.ParamEntry> formalParams = procEntry.getType().getFormalParams();
            if (formalParams.size() != node.getActualParams().size()) {
                staticError("wrong number of parameters", node.getLocation());
            } else {
                List<ExpNode> newActualParams = new ArrayList<>();
                for (int i = 0; i <  node.getActualParams().size(); i++) {
                    ExpNode param = node.getActualParams().get(i).transform(this);
                    if (formalParams.get(i).isRef()) {
                        newActualParams.add(formalParams.get(i).getType().coerceExp(param));
                    } else {
                        newActualParams.add(formalParams.get(i).getType().optDereferenceType()
                                .coerceExp(param));
                    }
                }
                node.setActualParams(newActualParams);
            }
        } else {
    ...
    public ExpNode visitSetNode(ExpNode.SetNode node) {
        beginCheck("Set");
        Type.SetType type = node.getType().resolveType().getSetType();
        if (type == null) {
            staticError("Expected set type", node.getLocation());
        } else {
            node.setType(type);
            List<ExpNode> newElements = new ArrayList<>();
            for (ExpNode elem : node.getElements()) {
                ExpNode newElement = elem.transform(this);
                newElements.add(type.getElementType().coerceExp(newElement));
            }
