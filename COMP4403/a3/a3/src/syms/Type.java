...
public abstract class Type {
    ...
    ExpNode coerce(ExpNode exp) throws IncompatibleTypes {
        throw new IncompatibleTypes(
                "type should be " + this.getName() + " not " + exp.getType().getName(),
                exp.getLocation());
    }
    ...
    public static class SetType extends Type {
        ...
        public void addOperators(Scope scope) {
            // T x S where S = setof(subrange(T, lower, upper))
            Type.ProductType inOpArgType
                    = new Type.ProductType(this.elementType.optWidenSubrange(), this);
            // T x S -> boolean
            Type.FunctionType inOpFnType
                    = new Type.FunctionType(inOpArgType, Predefined.BOOLEAN_TYPE);
            scope.addOperator(Operator.IN_OP, loc, inOpFnType);

            // S x S
            Type.ProductType binSetArgType = new Type.ProductType(this, this);
            // S x S -> boolean
            Type.FunctionType binSetComparisonType
                    = new Type.FunctionType(binSetArgType, Predefined.BOOLEAN_TYPE);
            scope.addOperator(Operator.EQUALS_OP, loc, binSetComparisonType);
            scope.addOperator(Operator.NEQUALS_OP, loc, binSetComparisonType);

            // S x S -> S
            Type.FunctionType binSetOperationType
                    = new Type.FunctionType(binSetArgType, this);
            scope.addOperator(Operator.ADD_OP, loc, binSetOperationType);
            scope.addOperator(Operator.MUL_OP, loc, binSetOperationType);
            scope.addOperator(Operator.SUB_OP, loc, binSetOperationType);

            // S -> S
            Type.FunctionType unaryOp = new Type.FunctionType(this, this);
            scope.addOperator(Operator.COMPLEMENT_OP, loc, unaryOp);
        }
