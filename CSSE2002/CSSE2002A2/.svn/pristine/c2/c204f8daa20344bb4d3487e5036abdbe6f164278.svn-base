...
public class Stop implements Writeable {
     ...
     *         3. Any of the parts of the string being missing.
     */
    public static Stop decode(String stopString)
        ...
        // split is [name, x, y]. A limit of -1 includes empty strings to check
        // if there is an extra : at the end of the string
        String[] split = stopString.split(":", -1);
        // If there are more than 3 arguments, throw an exception
        if (split.length != 3) {
            throw new TransportFormatException();
        }
        // First element renamed for readability
        String name = split[0];

        int x, y;
        try {
            // Removes leading and trailing spaces, then attempts to convert to
            // integers
            x = Integer.parseInt(split[1].trim());
            y = Integer.parseInt(split[2].trim());
        // If the strings for x and y cannot be parsed as ints, throw a
