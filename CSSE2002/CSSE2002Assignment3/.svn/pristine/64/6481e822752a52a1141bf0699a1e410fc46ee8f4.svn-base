...
public class RoutingEntry {
    // The next stop to go to
    private Stop nextStop;

    // The cost to get to the destination from this stop
    private int destinationCost;

    /**
     * Creates a new default RoutingEntry object. The next stop should be stored
     * as null, and the cost should be stored as Integer.MAX_VALUE.
     * These default values are used to represent a destination in a routing
     * table to which there is currently no known path.
     */
    public RoutingEntry() {
        this.nextStop = null;
        this.destinationCost = Integer.MAX_VALUE;
    }
     ...
     */
    public Stop getNext() {
        // If there is no known path, this.nextStop is initialised as null,
        // meaning this method does not have to consider that as a special case
